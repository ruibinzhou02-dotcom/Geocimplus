...
再次运行 `./start.ps1` 即可恢复。筛选和相机状态仅保存在当前页面，刷新回到初始状态；数据、报告和稳定ID映射保存在磁盘。出错查看 `reports/server.stderr.log`。端口被其他程序占用时请先解决冲突，不要直接结束不明进程。

## 可用功能

- 真实建筑二维/三维、height 离地高度着色、数值阈值筛选和ID检索。
- 道路显示、单期完整人口网格专题图、灰度卫星底图切换。
- 点击查看原始属性及持久稳定ID，缩放、旋转、倾斜、复位和清除筛选。
...
“超过50米”使用严格 `height > 50`，不是 `>=50`；最低/最高阈值均为开区间。统计针对全部符合筛选的记录，不是当前可见屏幕范围。高度统计排除null，0保持为0。隐藏建筑图层不会把符合条件的记录数改成0。
...
验证：`pnpm test` 直接依据本地审计数据核对真实筛选、ID映射、空结果及非法参数处理。没有本地目录和真实数据时这些验证会明确失败，而不是生成模拟结果。
