...
V2.2 测试入口：https://geocimplus.com/v2.html 。固定沙头示例在服务器分析，新建项目及用户数据在浏览器本地分析；展示、分析、结果三类图层已经接入。使用、目录授权、部署和回滚见 [V2.2 使用与交付](docs/V2.2使用与交付.md)。源码分支为 `codex/v2`，域名根首页仍保留 V1。

本轮新增 GIS 数据目录识别、路网、平面卫星底图、中英文流程与可连接数字滑条，见 [数据导入与数字电池](docs/V2.2数据导入与数字电池.md)。

以下内容描述 V1；V2.2 的行为和限制以新版交付说明为准。
