...
export default function UploadPanel({onImported,onError}) {
 ...
 return <div className="upload-panel"><h3>{t("上传数据")}</h3><label className="field-label">{t("数据类型")}<select aria-label={t("上传数据类型")} disabled={busy} value={kind} onChange={e=>{setKind(e.target.value);setFiles([]);setSuccess('');}}><option value="vector">{t("Shapefile 图层")}</option><option value="raster">{t("卫星影像")}</option></select></label>
 <label className={'upload-zone '+(busy?'busy':'')} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(!busy)setFiles([...e.dataTransfer.files]);}}><Upload size={27}/><b>{t("选择文件或拖放到这里")}</b><span>{t(kind==='vector'?'ZIP，或同时选择 shp / shx / dbf / prj':'GeoTIFF，或完整影像数据集 ZIP')}</span><input aria-label={t("选择上传文件")} type="file" multiple disabled={busy} accept={kind==='vector'?'.zip,.shp,.shx,.dbf,.prj,.cpg,.sbn,.sbx,.xml':'.zip,.tif,.tiff,.png,.jpg,.jpeg,.prj,.tfw,.pgw,.jgw,.wld'} onChange={e=>{setFiles([...e.target.files]);setSuccess('');e.target.value='';}}/></label>
 ...
 <p className="help">{t("文件保存在本机。单次最大 256 MiB；影像需带地理坐标。ZIP 可保留配套目录结构。")}</p>
