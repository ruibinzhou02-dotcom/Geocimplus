...
function App(){
 ...
 const exporter=useRef(null),[exportBusy,setExportBusy]=useState(false);
 const exportPNG=async()=>{setExportBusy(true);try{await exporter.current?.({title:t('项目：沙头分析'),note:catalog?.synthetic?t('基础图层：模拟数据'):t('本地真实数据'),stats:`${t('建筑数量')}: ${stats.count} | ${t('平均高度')}: ${stats.mean?.toFixed(1)} m`,lang,color});}catch(e){setError(e.message);}finally{setExportBusy(false);}};
 ...
 useEffect(()=>{(async()=>{
  ...
  const images=c.satellite?[{id:'satellite',name:'卫星底图',display:'灰度',url:'/api/satellite.png',coordinates:c.satellite.coordinates}]:[],nextSettings={...initialSettings};
 ...
 return <div className="app" style={{'--panel-alpha':1-panelTransparency/100}}><header className="topbar"><div className="brand"><div className="brand-icon"><Layers3 size={24}/></div><b>GeoCIM</b></div><div className="top-meta"><span className="local-dot"/>{t(catalog?.synthetic?"公开演示":"本地工作空间")}<button className="ai-settings-button" onClick={()=>{setTab('ai');refreshAI();}}>{t("AI 设置")}</button><div className="language-switch" aria-label={t('切换语言')}><button aria-pressed={lang==='zh'} onClick={()=>setLang('zh')}>{t("中文")}</button><button aria-pressed={lang==='en'} onClick={()=>setLang('en')}>English</button></div><button className="upload-top" onClick={()=>setTab('upload')}><Upload size={15}/>{t("上传数据")}</button></div></header>
 ...
 <div className="map-title"><h1>{t('项目：沙头分析')}</h1>{catalog?.synthetic&&<p className="demo-badge">{t('基础图层：模拟数据')}</p>}</div>
 <div className="map-toolbar"><button className={threeD?'selected':''} onClick={()=>setThreeD(!threeD)}>{t(threeD?'3D 建筑':'2D 平面')}</button><button disabled={!catalog?.satellite} className={settings.satellite?.visible?'selected':''} onClick={()=>toggle('satellite',!settings.satellite?.visible)}>{t("卫星底图")}</button><button disabled={!ready||exportBusy} onClick={exportPNG}>{t(exportBusy?"正在导出…":"导出 PNG")}</button><button aria-label={t("重置视角")} title={t("重置视角")} onClick={reset}><RotateCcw size={17}/></button></div>
 ...
 </section><aside><div className="aside-heading"><div><Layers3 size={21}/><h2>{t("探索助手")}</h2></div><select className="mode-select" aria-label={t('助手模式')} value={aiMode} disabled={aiBusy} onChange={e=>setAiMode(e.target.value)}><option value="local">{t(catalog?.synthetic?'演示指令':'本地预设')}</option><option disabled={catalog?.synthetic} value="openai">OpenAI API</option></select></div><nav className="tabs">{tabs.map(([key,Icon,label])=><button key={key} className={tab===key?'active':''} onClick={()=>{setTab(key);if(key==='attributes')setTableOpen(true);}}><Icon size={15}/>{t(label)}</button>)}</nav>
