...
function App(){
 ...
 const [input,setInput]=useState(''),[messages,setMessages]=useState([{role:'assistant',text:'可以打开属性表，按字段选择要素，再调整图层或选中要素的透明度。也可以上传自己的 Shapefile 和卫星影像。'}]);
 useEffect(()=>{(async()=>{
 ...
 const send=text=>{if(!text.trim())return;setInput('');let answer;try{
  const q=text.trim(),height=q.match(/^(?:选择|选中)(?:建筑)?高度(?:超过|大于)(\d+(?:\.\d+)?)米(?:的建筑)?$/),alpha=q.match(/^建筑透明度(?:设为|调为)?(\d+)%$/);
 ...
 const layerControls=key=><div className="layer-row" key={key}><Toggle label={metadata[key]?.name||key} checked={!!settings[key]?.visible} onChange={v=>toggle(key,v)}/><p>{dataset[key].features.length.toLocaleString()} 个要素</p><Transparency label={(metadata[key]?.name||key)+'透明度'} value={settings[key]?.transparency??30} onChange={v=>opacity(key,v)}/><div className="layer-actions"><button onClick={()=>openTable(key)}>属性表</button><button onClick={()=>locate(dataset[key].features)}>定位图层</button></div></div>;
 const panelControl=<Transparency label="统计卡与图例透明度" value={panelTransparency} onChange={setPanelTransparency}/>;
 return <div className="app" style={{'--panel-alpha':1-panelTransparency/100}}><header className="topbar"><div className="brand"><div className="brand-icon"><Layers3 size={24}/></div><b>GeoCIM</b><span className="brand-divider"/><span className="product-name">城市空间探索</span></div><div className="top-meta"><span className="local-dot"/>本地工作空间<button className="upload-top" onClick={()=>setTab('upload')}><Upload size={15}/>上传数据</button></div></header>
 ...
 <div className="map-title"><span className="eyebrow">URBAN SPATIAL EXPLORER</span><h1>{catalog?.title||'城市切片'}</h1><p>建筑 · 路网 · 人口 · 遥感影像</p></div>
 <div className="map-toolbar"><button className={threeD?'selected':''} onClick={()=>setThreeD(!threeD)}>{threeD?'3D 建筑':'2D 平面'}</button><button className={settings.satellite?.visible?'selected':''} onClick={()=>toggle('satellite',!settings.satellite?.visible)}>卫星底图</button><button aria-label="重置视角" title="重置视角" onClick={reset}><RotateCcw size={17}/></button></div>
 <div className="stats-bar"><div><span>建筑数量</span><strong>{stats.count.toLocaleString()}</strong></div><div><span>平均高度</span><strong>{stats.mean?.toFixed(1)??'—'}<small>米</small></strong></div><div><span>最高建筑</span><strong>{stats.max??'—'}<small>米</small></strong></div></div>
 <div className="legend"><b>建筑离地高度</b><span className="legend-unit">米</span>{color==='height'?<><div className="legend-ramp"/><div className="legend-ticks"><span>0</span><span>30</span><span>60</span><span>100</span><span>200+</span></div></>:<div className="uniform-key"/>}{settings.population?.visible&&<div className="pop-legend"><b>网格人数</b><div className="pop-ramp"/><div className="legend-ticks"><span>0</span><span>300</span><span>700</span><span>1,500</span><span>3,000+</span></div></div>}</div>
 {!ready&&<div className="loading">{error||'正在加载城市空间…'}</div>}
 {picked&&<div className="property-card"><div className="property-heading"><b>{metadata[picked.layer]?.name}属性</b><button aria-label="关闭要素属性" onClick={()=>setPicked(null)}><X size={16}/></button></div><dl>{Object.entries(picked.feature.properties).map(([k,v])=><React.Fragment key={k}><dt>{k}</dt><dd>{v==null?'NULL':String(v)}</dd></React.Fragment>)}</dl><button className="secondary" onClick={()=>{select({layer:picked.layer,ids:[featureId(picked.feature,metadata[picked.layer]?.idField||'stable_id')]});openTable(picked.layer);}}>在属性表中选择</button></div>}
 ...
 </section><aside><div className="aside-heading"><div><Layers3 size={21}/><h2>探索助手</h2></div><span className="mode-pill">本地预设</span></div><nav className="tabs">{tabs.map(([key,Icon,label])=><button key={key} className={tab===key?'active':''} onClick={()=>{setTab(key);if(key==='attributes')setTableOpen(true);}}><Icon size={15}/>{label}</button>)}</nav>
 <div className="aside-scroll">{error&&<div className="error" role="alert"><button aria-label="关闭提示" onClick={()=>setError('')}><X size={14}/></button>{error}</div>}
 {tab==='dialog'&&<><div className="context-card"><div className="context-icon"><Building2 size={21}/></div><div><b>{catalog?.title||'城市切片'}</b><p>{Object.keys(dataset).length} 个矢量图层 · 本地数据</p></div></div><div className="chat-messages">{messages.map((m,i)=><div key={i} className={'message '+m.role}>{m.role==='assistant'&&<span className="message-label">GEOCIM</span>}<p>{m.text}</p></div>)}</div><div className="suggestions"><span>从一个操作开始</span>{['打开属性表','选择高度超过50米的建筑','建筑透明度70%','显示人口网格'].map(t=><button key={t} onClick={()=>send(t)}>{t}<ArrowUp size={14}/></button>)}</div></>}
 {tab==='layers'&&<><h3>矢量图层</h3>{Object.keys(dataset).map(layerControls)}<h3>底图</h3>{rasters.map(r=><div className="layer-row" key={r.id}><Toggle label={r.name+(r.display?.includes('灰度')?'（灰度）':'')} checked={!!settings[r.id]?.visible} onChange={v=>toggle(r.id,v)}/><Transparency label={r.name+'透明度'} value={settings[r.id]?.transparency??0} onChange={v=>opacity(r.id,v)}/></div>)}<h3>浮动面板</h3>{panelControl}</>}
 {tab==='attributes'&&<><h3>图层属性表</h3><div className="attribute-layer-list">{Object.keys(dataset).map(key=><button className={key===tableLayer?'selected':''} key={key} onClick={()=>openTable(key)}><Table2 size={16}/><span>{metadata[key]?.name}</span><small>{dataset[key].features.length.toLocaleString()}</small></button>)}</div><div className="selection-summary"><b>{metadata[tableLayer]?.name}</b><span>已选 {selection.layer===tableLayer?selection.ids.length.toLocaleString():0} 个要素</span></div><Transparency label="当前图层透明度" value={settings[tableLayer]?.transparency??30} onChange={v=>opacity(tableLayer,v)}/><Transparency label="当前选中要素透明度" value={settings[tableLayer]?.selectedTransparency??15} disabled={selection.layer!==tableLayer||!selection.ids.length} onChange={v=>opacity(tableLayer,v,'selected')}/><p className="help">在下方属性表中按字段选择，也可以勾选单条记录。选中的要素以金色显示。</p><h3>建筑着色</h3><select aria-label="建筑着色方式" value={color} onChange={e=>setColor(e.target.value)}><option value="height">按离地高度着色</option><option value="uniform">统一颜色</option></select><h3>浮动面板</h3>{panelControl}</>}
 {tab==='upload'&&<UploadPanel onImported={imported} onError={setError}/>}
 </div><div className="composer"><form onSubmit={e=>{e.preventDefault();send(input);}}><textarea aria-label="输入地图指令" placeholder="例如：选择高度超过50米的建筑" rows={2} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send(input);}}}/><button type="submit" aria-label="发送指令"><ArrowUp size={17}/></button></form><p>本地预设指令</p></div></aside></main></div>;
}
