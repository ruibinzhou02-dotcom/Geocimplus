export function runJob(payload,onProgress=()=>{}){
 ...
 const promise=new Promise((resolve,reject)=>{
  ...
  worker.onmessage=({data})=>{if(data.progress!=null){onProgress(data.progress);return;}clearTimeout(timer);worker.terminate();data.error?reject(new Error(data.error)):resolve(data.result);};
